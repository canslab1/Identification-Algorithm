...
show_the_detailed_result            = False
show_the_major_result               = True
show_the_major_clustering_result    = False
show_the_detailed_clustering_result = False
    ...
    if os.path.exists(path) & os.path.isfile(path):
        ...
        for g in nx.connected_component_subgraphs(G):
            ...
            if path in SPECIAL_NETWORKS:
                pos = {seq_no:(float(g.node[seq_no]['posx']), float(g.node[seq_no]['posy'])) for seq_no in g.nodes_iter()}
            else:                
                pos = nx.spring_layout(g, pos = nx.circular_layout(g))
            if show_the_major_result == True:
                fig_no = 10
                net_fig1 = plot.figure(fig_no, figsize = (4, 6) if path in SPECIAL_NETWORKS else (6, 6), dpi = 200, facecolor = 'white')
                if path in SPECIAL_NETWORKS:
                    bb_width = [0.1 if g[s][t][EDGE_KEY_LAYER + n] == BOND else 0.5 for (s, t) in g.edges_iter(data = False)]
                ...
                if path in SPECIAL_NETWORKS:
                    nx.draw_networkx(g, with_labels = False, pos = pos, node_size = ns, linewidths = 0.5, edge_color = bb_color, width = bb_width)
                else:
            ...
            if show_the_detailed_result == True:
                for i in xrange(layers):
                    ...
                    net_sub_fig = plot.figure(fig_no + l, figsize = (12, 8), facecolor = 'white')
                    plot.title('target network = ' + tail + ' (layer ' + str(l) + ', R1 = ' + str(round(g.graph[GRAPH_KEY_THRESHOLD_R1 + str(i + 1)], 4)) +
                    ...
                    plot.savefig(root + '_' + str(compNo) + '_result_layer_' + str(l) + '.png')
            ...
            if show_the_betweenness_result == True:
                ...
                plot.title('Target network = ' + tail + ' (betweenness centrality for edges)')
                plot.xticks(())
                plot.yticks(())
                ...
                plot.savefig(root + '_' + str(compNo) + '_result (edge betweenness).png')
            ...
            if show_the_pagerank_result == True:
                ...
                plot.title('Target network = ' + tail + ' (pagerank-based weighting for edges)')
                plot.xticks(())
                plot.yticks(())
                ...
                plot.savefig(root + '_' + str(compNo) + '_result (pagerank-based).png')
            ...
            if show_the_major_clustering_result == True:
                ...
                net_fig4 = plot.figure(fig_no, figsize = (12, 8), facecolor = 'white')
                plot.title('Target network = ' + tail + ' (clustering result)')
                plot.xticks(())
                plot.yticks(())
                net_fig4.set_tight_layout(True)
                nx.draw_networkx(g, pos = pos, linewidths = 0, width = bb_width, node_color = ncc, vmin = min(ncc), vmax = max(ncc), cmap = plot.cm.Dark2, font_size = 8, edge_color = bb_color)
                plot.savefig(root + '_' + str(compNo) + '_result (network clustering).png')
                if show_the_detailed_clustering_result == True:
                    for key, sub_g in sg.items():
                        if type(sub_g) == list:
                            ...
                            show_g = sub_g
                        fig_no += 1
                        net_sub_fig = plot.figure(fig_no, figsize = (12, 8), facecolor = 'white')
                        plot.title('Target network = ' + tail + ' (' + key + ')')
                        plot.xticks(())
                        plot.yticks(())
                        net_sub_fig.set_tight_layout(True)
                        nx.draw_networkx(show_g, pos = pos, linewidths = 0, font_size = 8)
                        plot.savefig(root + '_' + str(compNo) + '_result (' + key + ').png')
...
def suite_experiment (suite = 'WS_SWN', onlyShow = True):    
    ...
    if os.path.exists(nfp_shelve_path) & os.path.isfile(nfp_shelve_path):
        ...
        net_fig1 = plot.figure(10, figsize = (12, 8), facecolor = 'white')
        p1       = plot.bar(index, bar[BOND],          width, color = BOND_COLOR,          edgecolor = BOND_COLOR)
        p2       = plot.bar(index, bar[LOCAL_BRIDGE],  width, color = LOCAL_BRIDGE_COLOR,  edgecolor = LOCAL_BRIDGE_COLOR,  bottom = bar[BOND])
        p3       = plot.bar(index, bar[GLOBAL_BRIDGE], width, color = GLOBAL_BRIDGE_COLOR, edgecolor = GLOBAL_BRIDGE_COLOR, bottom = bar[BOND] + bar[LOCAL_BRIDGE])
        p4       = plot.bar(index, bar[SINK],          width, color = SINK_COLOR,          edgecolor = SINK_COLOR,          bottom = bar[BOND] + bar[LOCAL_BRIDGE] + bar[GLOBAL_BRIDGE])
        plot.gca().xaxis.tick_top()                
        plot.xticks(index + width / 2., labels, rotation = 90)
        plot.ylabel('Percentage')
        plot.yticks(numpy.arange(0, 1.01, 0.1))
        plot.ylim(0, 1.)
        plot.legend((p1[0], p2[0], p3[0], p4[0]), (BOND, LOCAL_BRIDGE, GLOBAL_BRIDGE, SINK), loc = 'lower center', fancybox = True, shadow = True, ncol = 4)
        net_fig1.set_tight_layout(True)
        for t in plot.gca().xaxis.get_major_ticks(): t.tick1On = t.tick2On = False
        for t in plot.gca().yaxis.get_major_ticks(): t.tick1On = t.tick2On = False                
        plot.savefig('fingerprints_' + suite + '.png')
 
        sub_fig2, ax = plot.subplots()        
        ...
        ccmap        = ax.pcolor(corr_coef, vmin = -1.0, vmax = 1.0, cmap = plot.cm.RdBu, alpha = 0.8)
        cbar         = plot.colorbar(ccmap)
        sub_fig2     = plot.gcf()
        ...
        for t in plot.gca().xaxis.get_major_ticks(): t.tick1On = t.tick2On = False
        for t in plot.gca().yaxis.get_major_ticks(): t.tick1On = t.tick2On = False
        plot.savefig('correlation coeficient of complex networks_' + suite + '.png')

        net_fig3 = plot.figure(12, figsize = (12, 8), facecolor = 'white')
        corr_cluster = hc.dendrogram(hc.linkage(corr_matrix, method = 'centroid'), no_plot = False, labels = labels)
        plot.xticks(rotation = 90)
        plot.yticks(())
        net_fig3.set_tight_layout(True)
        plot.savefig('hierarchy of complex networks_' + suite + '.png')
